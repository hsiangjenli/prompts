...
- 確認要轉綠的測試項目與預期實作位置，並再次檢查 Test ID 是否符合模板或專案規範
- **重要**：確保功能 ID 與 Test ID 的對應關係正確，避免跨功能誤用
...
- 記錄以下內容：
  - Green 階段時戳與通過狀態（依模板或專案慣例使用的符號/說明）
  - 實作修改重點（檔案、改動摘要）
  - 測試通過的完整證據（測試輸出或截圖）
...
- 找到對應的 Test ID 行
- 將狀態欄位從原本的狀態更新為對應的通過狀態，並依需求加入 Comment 連結
