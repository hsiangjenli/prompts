---
mode: agent  
description: 透過問答方式釐清系統設計與規格需求，將 BDD 使用者故事轉為具體的技術規範與介面設計  
inputs:
  summary: 使用 Prompt 透過互動式問答釐清系統設計  
  required:
    - 直接對話進行釐清  
    - 提供 BDD Issue 編號（必填）、設計文件、架構圖、現有規格文件等相關資訊  
  optional:
...
outputs:
  ...
  include:
    ...
    - 列出規格設計、介面規範、資料模型（包含驗證方式與 Mock 策略）  
...
以不斷提問的方式釐清系統設計需求，並將 BDD 使用者故事轉換為具體的技術規範、介面規格與資料模型，方便後續 TDD 實作。
...
| 使用者提供的資訊不足以建立設計規格 | 持續提問直到至少有一個完整的介面規格或資料模型 |
