...
2. **重要**：確認 TDD Issue 的功能 ID（或模板要求的其他識別資訊），確保後續所有 Test ID 與模板規範一致
3. 從測試矩陣選定此次要處理的 Test ID（通常按優先順序 P0 → P1 → P2）
   - 檢查 Test ID 命名是否符合模板定義，維持追蹤唯一性
...
2. **為該 Test ID 建立或追加 Comment**：
   ...
   **首次執行 tdd-red 時（建立新 Comment）**：
   ...
   - 記錄以下 6 項必要資訊：
     ...
     - 測試檔案路徑
       - Red 階段時戳與失敗狀態（依模板或專案習慣使用的符號/說明）
     - 失敗原因簡述（錯誤訊息前 120 字）
     - 完整的錯誤訊息與堆疊
     - CI/CD 執行連結或本地輸出
...
3. **更新 TDD Issue 測試矩陣的狀態欄位**：
   - 找到對應的 Test ID 行
   - 將狀態欄位從模板定義的初始狀態更新為對應的失敗狀態，並依需求加入 Comment 連結
